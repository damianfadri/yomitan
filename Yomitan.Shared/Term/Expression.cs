...
namespace Yomitan.Shared.Term
{
    public class Expression
    {
        ...
        {
            string convertedText = JapaneseUtils.ConvertKatakanaToHiragana(Text);
            string convertedReading = JapaneseUtils.ConvertKatakanaToHiragana(Reading);
        ...
        {
            if (JapaneseUtils.IsKana(expression))
            {
            ...
            IList<string> expressionSegment = JapaneseUtils.Segment(expression).ToList();

            foreach (string segment in expressionSegment)
            {
                pattern.Append("(");
                if (JapaneseUtils.IsKana(segment))
